...
const bodiesOutputFile = path.join(root, "src", "content", "archive-bodies.ts")
...
const ARCHIVE_FORMATS = ["essay", "video", "podcast", "short-film", "commercial", "photo-set", "tool"]
const MIN_BODY_TEXT_LENGTH = 180
...
function deriveFormat(archive, project) {
  const series = archive.project ?? project?.slug
  if (archive.category === "Photography" || archive.entryType === "Photo") return "photo-set"
  if (archive.category === "Tools" || archive.entryType === "Tool" || archive.entryType === "Prototype") return "tool"
  if (series === "journey-tellers-podcast") return "podcast"
  if (series === "commercials") return "commercial"
  if (series === "short-films" || series === "music-videos") return "short-film"
  if (archive.platform === "YouTube") return "video"
  if (archive.platform === "Substack" || archive.platform === "Linkedin" || archive.platform === "Twitter") return "essay"
  if (archive.category === "Films & Commercials") return "short-film"
  if (archive.category === "Vlogumentaries") return "video"
  return "essay"
}
...
function normalizeArchive(archive, projectBySlug) {
  ...
  const format = ARCHIVE_FORMATS.includes(archive.format) ? archive.format : deriveFormat(archive, linkedProject)
  const image = archive.image || linkedProject?.image
  const dek = archive.dek || firstSentence(archive.summary || archive.title)
  ...
  return {
    ...
    seoTitle: archive.seoTitle || `${archive.title} | Archives | M Hadi`,
    seoDescription: archive.seoDescription || archive.summary || dek,
  }
}
...
function assertArchiveContent(archives) {
  for (const archive of archives) {
    ...
    assertEnum(archive.format, ARCHIVE_FORMATS, "format", archive)

    const bodyLength = htmlToText(archive.html).length
    if (bodyLength < MIN_BODY_TEXT_LENGTH) {
      throw new Error(`${id} body is too thin (${bodyLength} chars, need ${MIN_BODY_TEXT_LENGTH}). Write real page content.`)
    }

    const needsEmbed = ["video", "podcast", "short-film", "commercial"].includes(archive.format)
    if (needsEmbed && archive.platform === "YouTube") {
      const hasId = /[?&]v=([^&]+)/.test(archive.href) || /youtu\.be\/([^?]+)/.test(archive.href)
      if (!hasId) throw new Error(`${id} is a ${archive.format} but its href has no playable YouTube id`)
    }
  }
}
...
assertArchiveContent(archives)

// Archive bodies (rendered markdown) are by far the heaviest part of the
// content. They are emitted into a separate module that only the archive
// entry page imports (dynamically), keeping them out of the main bundle.
const archiveIndex = archives.map((archive) => {
  const copy = { ...archive }
  delete copy.html
  return copy
})
const archiveBodies = Object.fromEntries(archives.map((archive) => [archive.slug, archive.html]))
...
export const archives: ArchiveItem[] = ${JSON.stringify(archiveIndex, null, 2)}
`

const bodiesOutput = `// This file is generated by scripts/build-content.mjs. Do not edit by hand.
// Rendered markdown bodies for archive entries, keyed by slug. Loaded lazily
// by the archive entry page so this never lands in the main bundle.
export const archiveBodies: Record<string, string> = ${JSON.stringify(archiveBodies, null, 2)}
`

await fs.mkdir(path.dirname(outputFile), { recursive: true })
await fs.writeFile(outputFile, output)
await fs.writeFile(bodiesOutputFile, bodiesOutput)
